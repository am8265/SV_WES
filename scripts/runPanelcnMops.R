...
runPanelcnMops <- function(XandCB, testiv = c(1), countWindows, 
    ...
    XandCB@elementMetadata <- XandCB@elementMetadata[,c(testiv, (1:ncol(XandCB@elementMetadata))[-testiv])]
...
#    sampleThresh <- mean(sampleMedian[-testiv]) - 2*sd(sampleMedian[-testiv])
    ...
    if (length(poorQual) > 0) {
        ...
        saveRDS(countWindows[poorQual,], "poorQualExons.RDS")
    }
        ...
        if (length(selectedGenes) > 0) {
            ...
            for (g in selectedGenes) {
                ...
                if (length(geneIndTemp) == 0) {
                    poorSelected <- intersect(geneIndTemp, ignoreExons)
                    if (length(poorSelected) > 0) 
                        message(paste("Cannot use selected exon", countWindows[poorSelected,]$name, "\n"))
                }
            }
        }
