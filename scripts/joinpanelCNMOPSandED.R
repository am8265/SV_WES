...
#join EDs --outerJoin  #Note we  join on svtype --- 
  ...
  case.ED.joined = full_join(x = case.ED.match, y = case.ED.mixed, by = c("seqnames", "start", "end", "mcols.type"),
                             suffix = c(".matchG.ED", ".mixed.ED"))


# join panel.cnMOPS --outerJoin---we now use CN to join too 

  case.panelcnMOPS.joined = full_join(x= case.cnMOPS.match, y = case.cnMOPS.mixed, by = c("Chr", "Start", "End", "CN"), 
